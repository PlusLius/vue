...
//这些子节点应该也是 VNode 的类型。_createElement 接收的第 4 个参数 children 是任意类型的，
// 因此我们需要把它们规范成 VNode 类型。
...
export function normalizeChildren (children: any): ?Array<VNode> {
  ...
  return isPrimitive(children)
    ? [createTextVNode(children)] // 是原始类型的children说明是用户传入的children是一个简单的文本类型节点
    : Array.isArray(children) // 是数组可能是slot v-for这种情况
      ? normalizeArrayChildren(children) // 使用嵌套数组规范化的场景
      : undefined
}
// 检查是不是文本节点
...
// normalizeArrayChildren 主要的逻辑就是遍历 children，获得单个节点 c，然后对 c 的类型判断，如果是一个数组类型，
// 则递归调用 normalizeArrayChildren; 如果是基础类型，则通过 createTextVNode 方法转换成 VNode 类型；
// 否则就已经是 VNode 类型了，如果 children 是一个列表并且列表还存在嵌套的情况，则根据 nestedIndex 去更新它的 key。这里需要注意一点，
...
function normalizeArrayChildren (children: any, nestedIndex?: string): Array<VNode> {
  ...
  // ，获得单个节点 c
    ...
    last = res[lastIndex]
    //  nested
     // ，然后对 c 的类型判断，如果是一个数组类型，
    if (Array.isArray(c)) {
      if (c.length > 0) {
        // 则递归调用 normalizeArrayChildren; 
    ...
    } else if (isPrimitive(c)) { // 如果是基础类型，则通过 createTextVNode 方法转换成 VNode 类型；
      if (isTextNode(last)) {
        // merge adjacent text nodes
